...
<head>
    ...
    <link href="{{ asset('assets/css/sweditor.css')}}" rel="stylesheet">
...
<body class="flex flex-col min-h-screen @if(Request::is('/')){{ 'bg-white' }}@else{{ 'bg-gray-50' }}@endif">
    ...
    @if(session('message'))
        <script>setTimeout(function(){ popToast("{{ session('message_type') }}", "{{ session('message') }}"); }, 10);</scrip>
    @endif
    @waveCheckout
    <script src="{{ asset('assets/js/sweditor.js')}}"></script>
