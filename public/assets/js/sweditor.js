(function (window) {
  ...
  function calculateCharacters(elem) {
    if (elem.startsWith("<pre")) return 0;
    let text = elem.replace(/<[^>]*>/g, "");
    text = text.replaceAll("&nbsp;", "");
    text = text.replaceAll(/[\s]*&[^;]*;/g, ",");
    return text.replace(/\s+/g, "").length;
  }

  function calculateWords(elem) {
    if (elem.startsWith("<pre")) return 0;
    let text = elem.replaceAll("<div>", " ");
    text = text.replace(/<[^>]*>/g, "");
    text = text.replaceAll("&nbsp;", "");
  ...
  function calculateLines(elem) {
    if (elem.startsWith("<pre")) return 0;
    let text = elem.replaceAll("<div><br></div>", "");
    return text.split("<div>").length;
  }
  ...
  sweDitor.prototype = {
    ...
    _init: function () {
      ...
      this.selects = Array.prototype.slice.call(
        this.elem.querySelectorAll(".selectThis")
      );
      // array for click event
      this.formats = Array.prototype.slice.call(
        this.elem.querySelectorAll(".formatBlock")
      );
      // get print id
    ...
    _initEvents: function () {
      ...
      this.box.addEventListener(
        ...
        function () {
          ...
          // console.log(calculateCharacter(self.textArea.textContent));
          // console.log(calculateWords(self.textArea.textContent));
          // console.log(calculateLines(self.textArea.textContent));
          document.getElementById("count-characters").textContent =
            calculateCharacters(self.textArea.textContent);
          document.getElementById("count-words").textContent = calculateWords(
            self.textArea.textContent
          );
          document.getElementById("count-lines").textContent = calculateLines(
            self.textArea.textContent
          );
        },
        false
      ...
      // get print
      this.getPrint.addEventListener(
        "click",
        function () {
      ...
      // get clean
      this.getClean.addEventListener(
        "click",
        function () {
      ...
      // swich pre code
      this.switchBox.addEventListener(
        "change",
        function () {
